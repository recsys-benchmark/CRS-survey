...
class FactorizationMachine(nn.Module):
    def __init__(self, emb_size, user_length, item_length, feature_length, qonly, command, hs, ip, dr, old_new, interact):
    ...
    def forward_124(self, ui_pair, feature_index, preference_index):
        if self.interact==0:
            feature_matrix_ui = self.ui_emb(ui_pair)  # (bs, 2, emb_size+1), 
            nonzero_matrix_ui = feature_matrix_ui[..., :-1]  # (bs, 2, emb_size)
            feature_bias_matrix_ui = feature_matrix_ui[..., -1:]  # (bs, 2, 1)
            # _______ make a clone _______
            nonzero_matrix_clone = nonzero_matrix_ui.clone()
            feature_bias_matrix_clone = feature_bias_matrix_ui.clone()

            # _________ sum_square part _____________
            summed_features_embedding_squared = nonzero_matrix_ui.sum(dim=1, keepdim=True) ** 2  # (bs, 1, emb_size)

            # _________ square_sum part _____________
            squared_sum_features_embedding = (nonzero_matrix_ui * nonzero_matrix_ui).sum(dim=1, keepdim=True)  # (bs, 1, emb_size)
            ...
            feature_matrix_preference = self.feature_emb(preference_index) # (bs,19,emb_size+1), 19 is the largest padding

            # _______ dropout has been done already (when data was passed in) _______
            nonzero_matrix_preference = feature_matrix_preference[..., :-1]  # (bs, 19, emb_size)
            feature_bias_matrix_preference = feature_matrix_preference[..., -1:]  # (bs, 19, 1)

            # _______ concatenate them together ______
            nonzero_matrix = torch.cat((nonzero_matrix_ui, nonzero_matrix_preference), dim=1) # (bs, 21, emb_size)
            feature_bias_matrix = torch.cat((feature_bias_matrix_ui, feature_bias_matrix_preference), dim=1)# (bs, 21, 1)
            ...
            # !!!!!!attribute preference predict loss
            # ***---***
