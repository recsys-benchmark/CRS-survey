...
def PG_train(agent, load_model_type, dm): 
    ...
    sys.stdout = Logger("./data/{}/interaction-log/PG-agentear-{}-{}-{}-lr-{}.log" \
                        .format(data_name,data_name,load_model_type,date_str, str(PG_lr)))
    ...
    for _ in range(PG_epoch_num):
        ...
        for e_data in tqdm(random.sample(ui_train_list, 10000), ncols=0):
            ...
            epoch_success_sum += success #  success True: 1; False: 0
        ...
        if (_ + 1) % PG_save_step == 0:
            ...
            for e_data in tqdm(random.sample(ui_test_list, test_size), ncols=0):
                reward, turn, success, no_use, result = agent.PG_eva_one_episode(e_data[0], e_data[1])
                episode_num += 1
                sum_reward += reward
                sum_turn += turn
                sum_success += success
