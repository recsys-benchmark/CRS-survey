...
class ConfigLastFM:
    ...
    def _data_init(self):
        ...
        # connect attribute with interacted item (att_info) and uninteracted items (att_reverse_info)
        for item in item_info:
            new_item_info[int(item)] = set(item_info[item][ATT_KEY])# feature index

            if need_att_info:
                for att in item_info[item][ATT_KEY]:
                    if len(att_info.get(int(att), set())) == 0:
                        att_info[int(att)] = set()
                        att_info[int(att)].add(int(item)) # key：att, value: interacted itemID
                    else:
                ...
                for att in set(range(self.att_num)) - set(item_info[item][ATT_KEY]):
                    if len(att_reverse_info.get(int(att), set())) == 0:
                        ...
                        att_reverse_info[int(att)].append(int(item)) # key：att, value: no interacted itemID
                    else:
        ...
        for user in self.user_info:
            for item in self.user_info[user]:
                self.adj_index[0].append(user) # adj_index[0]: [u1,u+i1,u2,u+i2]
                self.adj_index[1].append(item + self.user_num)
                self.adj_index[1].append(user) # adj_index[1]: [u+i1,u1,u+i2,u2]
                self.adj_index[0].append(item + self.user_num)
        for item in self.item_info:
            for att in self.item_info[item]:
                self.adj_index[0].append(item + self.user_num) # adj_index[0]:[u1,u+i1,u2,u+i2,u+i1,att1+u+i,u+i2,att2+u+i]
                self.adj_index[1].append(att + self.item_num + self.user_num)
                self.adj_index[1].append(item + self.user_num) # adj_index[1]: [u+i1,u1,u+i2,u2,att1+u+i,u+i1,att2+u+i,u+i2]
        ...
        self.att_tree_dict = trans_index(attribute_tree_dict) #transform the key into int
