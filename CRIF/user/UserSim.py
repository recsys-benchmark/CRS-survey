...
class UserSim():
    ...
    def init_episode(self, user, target_item, given_init_parent_attribute):
        ...
        self.pos_attribute_set = self.item_info[target_item] # 所有的target_attribute

        if given_init_parent_attribute is None:
            # 把包含任一子positive attribute的 所有父attribute找出来，随机选一个初始的 parent attribute
            parent_att_list = []
        ...
        att_in_parent = self.attribute_tree[init_parent_att] # attribute value
        # 原始的positive attribute value和选出来的初始parent attribute的所有子attribute的交集是真正的初始positive attribute
        init_pos_attribute_set = self.pos_attribute_set & att_in_parent 
        # 差集是初始的negative attribute
    ...
    def next_turn(self, asked_attribute_list):
        pos_attribute_list = set(asked_attribute_list) & self.pos_attribute_set # 相交
