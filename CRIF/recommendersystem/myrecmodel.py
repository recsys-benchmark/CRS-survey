...
class MyRec(nn.Module):
    ...
    def get_item_score(self, user, pos_att_list, pos_att_list_mask, neg_att_list, neg_att_list_mask, item):
        user_feature = self.neg_user # 用neg item更新过后的user信息
        item_feature = self.graph_rep[self.item_graph_index[(item).type(torch.long)]]
    ...
    def get_item_list_score(self, user, pos_att_list, pos_att_list_mask, neg_att_list, neg_att_list_mask, item_list):
        ...
        item_feature = self.graph_rep[self.item_graph_index[(item_list).type(torch.long)]]
        user_feature = user_feature.unsqueeze(-2).expand(item_feature.size())
        user_item_score = torch.sum(user_feature * item_feature, dim=-1)
        user_item_score = self.tanh(user_item_score)
    ...
    def update_neg_user_embedding(self, user_list, pos_item_list, neg_item_list, neg_item_mask):
        ...
        neg_item_rep = self.graph_rep[self.item_graph_index[(neg_item_list).type(torch.long)]]
        ...
        for i, atts_list in enumerate(neg_item_att_list):
            for j, atts in enumerate(atts_list):
                if len(atts) != 0:
                    att_rep_list = self.graph_rep[self.attribute_graph_index[(atts).type(torch.long)]]
    ...
    def get_item_score_single(self, user, target_item, pos_att_list, neg_att_list, neg_item, item_list=None): # target_item = -1
        if neg_item == None:
            user_feature = self.get_user_embedding(user) # 问完调用的时候是这个，
            # convhis.py get_candidate_len_and_target_rank_for_ask ——> recsys.py get_item_preference
        else:
    ...
    def get_att_score(self, user, pos_att_list, pos_att_list_mask, neg_att_list, neg_att_list_mask,att):
        user_feature = self.get_user_embedding(user)
        att_feature = self.graph_rep[self.attribute_graph_index[(att).type(torch.long)]]
    ...
    def get_att_list_score(self, user, pos_att_list, pos_att_list_mask, neg_att_list, neg_att_list_mask,
                       att_list):
        user_feature = self.get_user_embedding(user)
        att_feature = self.graph_rep[self.attribute_graph_index[(att_list).type(torch.long)]]
        user_feature = user_feature.unsqueeze(-2).expand(att_feature.size())
        user_att_score = torch.sum(user_feature * att_feature, dim=-1)
        user_att_score = self.tanh(user_att_score)
    ...
    def get_att_item_score(self, att, item):
        att_rep = self.graph_rep[self.attribute_graph_index[(att).type(torch.long)]]
        item_feature = self.graph_rep[self.item_graph_index[(item).type(torch.long)]]
    ...
    def get_att_list_item_score(self, att_list, att_list_mask, item):
        att_rep = self.graph_rep[self.attribute_graph_index[(att_list).type(torch.long)]]
    ...
    def get_att_list_att_score(self, att_list, att_list_mask, score_att_list):
        ...
        score_att_feature = self.graph_rep[self.attribute_graph_index[(score_att_list).type(torch.long)]]
    ...
    def get_item_all_att_score(self, item, all_att):
        att_rep = self.graph_rep[self.attribute_graph_index[(all_att).type(torch.long)]]
        item_feature = self.graph_rep[self.item_graph_index[(item).type(torch.long)]]
