...
class DQN(nn.Module):
    def __init__(self, state_space, hidden_size, action_space):
        ...
        self.state_space = state_space#command
...
def train(args, kg, dataset, filename):
    ...
    env = EnvDict[args.data_name](kg, dataset, args.data_name, seed=args.seed, max_turn=args.max_turn,
                       cand_len_size=args.cand_len_size, attr_num=args.attr_num, mode='train', command=args.command, ask_num=args.ask_num, entropy_way=args.entropy_method, fm_epoch=args.fm_epoch,reward_pre=args.reward_pre)#original
    ...
    for i_episode in range(args.load_rl_epoch+1, args.epochs+1): #args.epochs
        ...
        for t in count():   # user  dialog
            ...
            next_state, reward, done ,recall_1,recall_5,recall_10,MRR_1,MRR_5,MRR_10= env.step(action.item())
        ...
        if i_episode % (args.observe_num * 4) == 0 and i_episode > 0:#
            ...
            dqn_evaluate(args, kg, dataset, agent, filename, i_episode)#i_episode
...
def main():
    ...
    # command:8   self.conver_his, self.cand_len, attribute_seq(same as crm)
    '''
    ...
    filename = 'train-data-{}-RL-command-{}-ask_method-{}-attr_num-{}-ob-{}-all_epoch-{}-fm_epoch-{}-reward_pre-{}'.format(
        args.data_name, args.command, args.entropy_method, args.attr_num, args.observe_num, args.epochs, args.fm_epoch,args.reward_pre)
