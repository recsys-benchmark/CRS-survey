...
def train(args, kg, dataset, filename):
    env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, seed=args.seed, max_turn=args.max_turn, cand_num=args.cand_num, cand_item_num=args.cand_item_num,
                       attr_num=args.attr_num, mode='train', ask_num=args.ask_num, entropy_way=args.entropy_method, fm_epoch=args.fm_epoch, reward_pre=args.reward_pre)
    ...
    for train_step in range(1, args.max_steps+1):
        ...
        recall_1,recall_5,recall_10,MRR_1,MRR_5,MRR_10=0,0,0,0,0,0#new
        ...
        for i_episode in tqdm(range(args.sample_times),desc='sampling'):
            ...
            for t in count():   # user  dialog
                ...
                if not args.fix_emb:
                    next_state, next_cand, action_space, reward, done, recall_1,recall_5,recall_10,MRR_1,MRR_5,MRR_10 = env.step(action.item(), sorted_actions, agent.gcn_net.embedding.weight.data.cpu().detach().numpy())
                else:
                    next_state, next_cand, action_space, reward, done, recall_1,recall_5,recall_10,MRR_1,MRR_5,MRR_10 = env.step(action.item(), sorted_actions)
                epi_reward += reward
...
def main():
    ...
    parser.add_argument('--reward_pre', type=int, default=0, help='use reward_pre or not')
    ...
    filename = 'train-data-{}-RL-cand_num-{}-cand_item_num-{}-embed-{}-seq-{}-gcn-{}-reward_pre-{}'.format(
        args.data_name, args.cand_num, args.cand_item_num, args.embed, args.seq, args.gcn,args.reward_pre)
