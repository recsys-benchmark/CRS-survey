...
class BinaryRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, embed, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0, reward_pre=0):
    ...
    def step(self, action, sorted_actions, embed=None):  
            ...
            for act in sorted_actions:
                if act < self.user_length + self.item_length:
                    ...
                    if len(recom_items) == self.rec_num:
                        break
            reward, done, self.target_item_position = self._recommend_update(recom_items)
            #========================================
            if reward > 0:#
        ...
        self.cur_conver_step += 1
        return self._get_state(), self._get_cand(), self._get_action_space(), reward, done,recall_1,recall_5,recall_10,MRR_1,MRR_5,MRR_10
